...
function Theory() {
  const { uploadedExcelData, uploadedHeaders, setOriginalExcelData } = useExcelData();
  ...
  const handleFileChange = (e) => {
    ...
    const convertIPData = (dataToConvert) => {
        return dataToConvert.map((row) => {
            ...
            return { "S.No": sno, "Reg. No.": regNo, "Name": name, "IP1": isIPAbsent ? "AB" : ip1, "IP2": isIPAbsent ? "AB" : ip2, "IP (16)": isIPAbsent ? "AB" : ip16Total };
        });
    };
    ...
    const convertIP1COAttainmentData = (ipData, lowestTest1CO) => {
        return ipData.map(row => {
            ...
            const marksAllocated = 8; // FIX: Corrected from 8 to 10
  ...
  const handleTest1Click = () => {
    ...
    navigate("/test1-results", { state: { convertedData: data } });
  };

  const handleTest2Click = () => {
    const data = convertTest2Data(uploadedExcelData);
    navigate("/test2-results", { state: { convertedData: data } });
  };

  const handleIPClick = () => {
    const data = convertIPData(uploadedExcelData);
    navigate("/ip-results", { state: { convertedData: data } });
  };
  ...
  const handleDownloadAllData = () => {
    ...
    XLSX.writeFile(wb, "All_Theory_Data_With_Summaries.xlsx");
  };


  return (
    <div className="p-6">
      <h2 className="text-2xl font-semibold mb-4">Upload Theory file</h2>
      ...
      <div className="w-60 mb-4">
        {/* MODIFIED: Added styling to the Upload button */}
        <Button
          label="Upload File"
      ...
      {displayExcelData.length > 0 && (
        <>
          <div className="mt-4 flex space-x-4">
            {/* MODIFIED: Added styling to Test 1, Test 2, and IP buttons */}
            <Button
              label="Test 1"
